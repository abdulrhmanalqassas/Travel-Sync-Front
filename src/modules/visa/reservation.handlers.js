...
async function getReservation(
  ...
  let data = await instance
    .get(
      `/api/visa-reservations${id ? `/${id}` : ""}?limit=50${status ? `&filters=${encodeURIComponent(JSON.stringify({ status: status }))}` : ""}`,
      {
        headers: {
...
async function getAllReservations(
  ...
  let data = await instance
    .get(
      `/api/visa-reservations?limit=${limit}&page=${page}${status ? `&filters=${encodeURIComponent(JSON.stringify({ status: status }))}` : ""}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
async function requestAction(setIsLoading, message, id = "", callback) {
  ...
  setIsLoading(true);
  let data = await instance
    .patch(`/api/visa-reservations/${id}/request-action`, message, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
async function cancelReservation(setIsLoading, message, id = "", callback) {
  ...
  setIsLoading(true);
  let data = await instance
    .patch(`/api/visa-reservations/${id}/cancel`, message, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
async function acceptReservation(setIsLoading, id = "", callback) {
  ...
  setIsLoading(true);
  try {
    let data = await instance.patch(`/api/visa-reservations/${id}/confirm`, null, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
async function Reserve(setIsLoading, values) {
  ...
  setIsLoading(true);
  try {
    let data = await instance.post(`/api/visa-reservations`, values, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
async function ReserveVisa(setIsLoading, values) {
  const cookie = Cookies.get("auth-token-data");
  const token = JSON.parse(cookie ? cookie : "null")?.token;
  setIsLoading(true);
  try {
...
async function uploadFile(file, isLoading, status, travellerIDs) {
  ...
  try {
    ...
    const result = await instance.post("/api/attatchments/upload", formData, {
      headers: {
